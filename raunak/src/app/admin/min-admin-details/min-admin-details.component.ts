...
@Component({
  selector: 'app-min-admin-details',
  templateUrl: './min-admin-details.component.html',
  styleUrls: ['./min-admin-details.component.scss']
})
export class MinAdminDetailsComponent { 
...
minAdminData(created_by:any){
  ...
  this.http.post<any>(url, created_by).subscribe((results: any) => {
    ...
    this.minAdminDataArray = results.filter((data: any) => {
      return data.level == 3;
    });
