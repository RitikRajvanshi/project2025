...
@Component({
  selector: 'app-supmaster-details',
  templateUrl: './supmaster-details.component.html',
  styleUrls: ['./supmaster-details.component.scss']
})
export class SupmasterDetailsComponent {
  ...
  getUserdata(){
    const user_data = localStorage.getItem('user_data');
      ...
      this.superMasterData(results[0]);
      })
  }
...
superMasterData(created_by:any){
  ...
  this.http.post<any>(url, created_by).subscribe((results: any) => {
    ...
    this.superMasterDataArray = results.filter((data: any) => {
      return data.level == 4;
    });
